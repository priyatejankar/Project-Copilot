...
def Decision_trees_test( train_set, label_train,  validation_set, label_validation, depth =8):
    print("Decision trees test")
    clf = tree.DecisionTreeRegressor(criterion='mse', max_depth=depth)
...
def Decision_trees_plot( train_set, label_train,  validation_set, label_validation, depth =8):
    print("Decision trees test")
    clf = tree.DecisionTreeRegressor(criterion='mse', max_depth=depth)
    clf = clf.fit(train_set, label_train)
    trees_computed_values = clf.predict(validation_set)
    ...
    train_scores, test_scores = validation_curve(tree.DecisionTreeRegressor(criterion='mse'), X, y,
    ...
    # print(test_scores)
    plt.title("DecisionTrees, Toy dataset (200 000 RTA), criterion : 'mse'")
...
# Decision_trees_plot( mel_train_values, mel_y_train_values,  test_values, y_test_values)
